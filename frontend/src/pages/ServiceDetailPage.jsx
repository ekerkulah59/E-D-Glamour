  ...
  if (!service) {
    return (
      <div className="min-h-screen pt-24 flex flex-col items-center justify-center">
        ...
        <Link to="/services"><Button className="rounded-full">Back to Services</Button></Link>
      </div>
    );
  }
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              <div className="aspect-[4/3] rounded-xl overflow-hidden">
                <img src={images[activeImage] || ''} alt={service.name} className="w-full h-full object-cover" data-testid="service-main-image" />
              </div>
              {images.length > 1 && (
                <div className="flex gap-3">
                  {images.slice(0, 3).map((img, idx) => (
                    <button key={`thumb-${idx}`} onClick={() => setActiveImage(idx)} className={`w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${activeImage === idx ? 'border-primary' : 'border-transparent opacity-60 hover:opacity-100'}`}>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div>
                ...
                <h1 className="font-heading text-3xl md:text-4xl font-bold text-foreground mb-4" data-testid="service-title">{service.name}</h1>
        ...
        <div className="container-custom">
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="font-heading text-2xl md:text-3xl font-bold text-foreground mb-3">Request a Quote</h2>
              <p className="font-body text-muted-foreground">Tell us about your event and we'll create a custom package for you.</p>
            </div>
            <div className="bg-white rounded-2xl p-8 shadow-sm">
              <QuoteForm serviceId={service.id} prefillEventType={service.category} />
            </div>
          </div>
        </div>
      </section>
