...
import { formatPrice, rentalCategoryLabels } from '../lib/utils';
...
const ImageThumbnail = ({ img, index, isActive, onClick }) => (
  <button
    ...
    className={`w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${
      isActive ? 'border-primary' : 'border-transparent opacity-60 hover:opacity-100'
    }`}
...
const SpecRow = ({ label, value }) => (
  <tr className="border-b border-border last:border-b-0">
    <td className="px-4 py-3 font-body text-sm font-medium capitalize">
      {label.replace(/_/g, ' ')}
    </td>
...
const RentalDetailPage = () => {
  ...
  const categoryLabel = rentalCategoryLabels[rental.category] || rental.category;
  const specKeys = Object.keys(rental.specifications);
              ...
              <div className="p-6 bg-muted rounded-xl">
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="font-heading text-3xl font-bold text-foreground" data-testid="rental-price-daily">
                  ...
                  {rental.price_per_week && (
                    <div>
                      ...
                      <p className="font-heading text-2xl font-bold text-primary" data-testid="rental-price-weekly">
                        {formatPrice(rental.price_per_week)}
                      </p>
                    </div>
                  )}
                </div>
                <p className="font-body text-xs text-muted-foreground mt-3">
                  Minimum rental: {rental.min_rental_days} day{rental.min_rental_days > 1 ? 's' : ''}
                </p>
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-body text-sm font-medium">
                      {rental.quantity_available} units available
                    </p>
                    <p className="font-body text-xs text-muted-foreground">
                      {rental.is_available ? 'Ready for rental' : 'Currently unavailable'}
                    </p>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="bg-white rounded-xl border border-border overflow-hidden">
                  <table className="w-full">
                    <tbody>
                      {specKeys[0] && <SpecRow label={specKeys[0]} value={rental.specifications[specKeys[0]]} />}
                      {specKeys[1] && <SpecRow label={specKeys[1]} value={rental.specifications[specKeys[1]]} />}
                      {specKeys[2] && <SpecRow label={specKeys[2]} value={rental.specifications[specKeys[2]]} />}
                      {specKeys[3] && <SpecRow label={specKeys[3]} value={rental.specifications[specKeys[3]]} />}
                      {specKeys[4] && <SpecRow label={specKeys[4]} value={rental.specifications[specKeys[4]]} />}
                    </tbody>
                  </table>
                </div>
              </div>
